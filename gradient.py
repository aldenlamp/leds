...
class Gradient:
    ...
    def __init__(self, colors=None, color_positions=None, 
        ...
        if color_positions is None:  
            ...
            divDist = self.GRAD_RANGE / len(colors)
    ...
    def get_color_at(self, position, brightness = 2/3, dim=False):
        ...
        if color_pos < self.color_positions[0] or \
           color_pos > self.color_positions[-1]:
            ...
            color_dist = self.GRAD_RANGE - self.color_positions[-1] + \
                         self.color_positions[0]
            lower = self.colors[-1]
            upper = self.colors[0]
            
            if color_pos < self.color_positions[0]:
                dist_from_left = self.GRAD_RANGE - self.color_positions[-1] + color_pos
            else:
